...
ALTER TABLE public.plans
  ADD COLUMN maintenance_per_mile numeric NOT NULL DEFAULT 0.32,    -- $ per mile driven, accrued every Friday
  ADD COLUMN tax_pct numeric NOT NULL DEFAULT 20,                   -- % of the month's pre-tax profit, off the top
  ADD COLUMN maintenance_floor numeric NOT NULL DEFAULT 5000,       -- floor 2 of the interlock (proposed, editable)
  ADD COLUMN min_move numeric NOT NULL DEFAULT 500,                 -- under it nothing moves
  ADD COLUMN objective_pct numeric NOT NULL DEFAULT 70,             -- the split; household gets the rest
  ADD COLUMN first_money_month date NOT NULL DEFAULT '2026-08-01';  -- the first month under these rules
...
ALTER TABLE public.plan_accounts DROP CONSTRAINT plan_accounts_role_check;
...
ALTER TABLE public.account_snapshots
  ...
  ADD COLUMN settles_month date;
CREATE UNIQUE INDEX account_snapshots_settles_month_key
