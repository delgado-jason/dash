...
const Card = ({
  ...
  <div className="ds2-board p-4">
    ...
    <p className="text-base font-condensed text-ink mt-1">{title}</p>
    <p className="text-sm mt-1">
      <span className={`font-semibold ${rpmTextClass(rpm)}`}>{fmtRpm(rpm)}</span>
      <span className="text-dim text-xs"> typical</span>
      <span className="text-dim"> · {loads} loads</span>
    </p>
    {blended !== null && (
...
const EmptyCard = ({ label, icon }: { label: string; icon: ReactNode }) => (
  <div className="ds2-board p-4">
    ...
    <p className="text-base font-condensed text-dim mt-1">—</p>
    <p className="text-sm text-dim mt-1">not enough data</p>
  </div>
);

const FLAME = <Flame size={14} style={{ color: "#e8621e" }} />;
const TRUCK = <Truck size={14} className="text-dim" />;
